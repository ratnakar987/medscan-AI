...
export const generateTextSummary = (report: any): string => {
  ...
  const diagnosis = report.potential_diagnosis_guess || analysis.potential_diagnosis_guess || 'Medical Report';
  ...
  const dietary = report.dietary_recommendations || analysis.dietary_recommendations || [];
  ...
  if (medicines.length > 0) {
    ...
    medicines.forEach((m: any) => {
      text += `- ${m.name}: ${m.dosage} (${m.timing || m.frequency})\n`;
    });
    text += `\n`;
  }
  ...
  if (dietary.length > 0) {
    ...
    dietary.forEach((d: any) => {
      text += `- ${d.food}: ${d.benefit}\n`;
    });
    text += `\n`;
  }
  
  text += `_Generated by AI Medical Assistant_`;
  return text;
};
