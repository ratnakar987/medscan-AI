...
const Layout: React.FC = () => {
  ...
  if (!user && !isLandingPage) return <Outlet />;

  return (
    <div className={cn("min-h-screen bg-white", !isLandingPage && "pb-24")}>
      {!isOnline && (
      ...
      <main className={cn(
        "mx-auto",
        isLandingPage ? "w-full" : "max-w-2xl px-4 pt-4"
      )}>
        <Outlet />
      </main>

      {!isLandingPage && user && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-100 px-8 py-4 flex justify-between items-center max-w-2xl mx-auto z-50">
          {navItems.map((item) => {
